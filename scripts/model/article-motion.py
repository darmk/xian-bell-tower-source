...
OUT=Path('/workspace/scratch/c0712837f0ef/wechat-frames')
W,H=960,800
for frame in range(96):
 theta=math.radians(42)+2*math.pi*frame/96
 eye=np.array([88*math.sin(theta),42,88*math.cos(theta)])
 target=np.array([0,15.5,0.]);forward=target-eye;forward/=np.linalg.norm(forward);right=np.cross(forward,[0,1,0]);right/=np.linalg.norm(right);up=np.cross(right,forward);R=np.stack([right,up,forward]);f=H/(2*np.tan(np.deg2rad(39)/2));depth=np.full((H,W),1e9);img=np.zeros((H,W,3),np.uint8)
 for y in range(H):img[y,:,:]=np.array([24,41,55])+(y/H)*np.array([7,8,8])
 for m,a in meshes:raster(*a,textures[m['mat']],R,eye,f,W,H,depth,img,False)
 Image.fromarray(img).save(OUT/f'{frame:04}.png')
 if frame%12==0:print('Turntable frame',frame,flush=True)
